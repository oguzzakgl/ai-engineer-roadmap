...
### NOTLAR:
- 1.1 Type Hinting (Tip Belirteçleri) modülü interaktif alıştırmalarla tamamlandı (faz1/typehinting.py).
- 1.1 Dosya İşlemleri & JSON Serialization/Deserialization konusu tamamlandı (faz1/dosya_islemleri.py ve faz1/json_islemleri.py).
- 1.1 Hata Yönetimi (try...except...else...finally & Custom Exceptions) konusu tamamlandı (faz1/hata_yonetimi.py).
- 1.1 Asenkron Python (asyncio - async/await & non-blocking I/O) konusu tamamlandı (faz1/asenkron.py).
